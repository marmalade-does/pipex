...
/*   Updated: 2025/07/15 16:17:42 by lroberts         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "pipex.h"

static void	handle_error(char **splitted_cmd, char **paths, char *msg, int code)
{
...
char	*get_path(char **splitted_cmd, char **envp)
{
	...
	while (envp[i])
	{
		if (ft_strncmp(envp[i], "PATH=", 5) == 0)
		{
			...
			if (paths == NULL)
				handle_error(splitted_cmd, NULL, "ft_split fail-get_path", 1);
			break ;
		}
		i++;
	}
	if (paths == NULL)
	{
		handle_error(splitted_cmd, NULL, "PATH not in env - get_path", 2);
	}
...
char	*ft_test_paths(char **splitted_cmd, char **paths)
{
	...
	if (!back_slashed_paths)
		handle_error(splitted_cmd, NULL, "app*_back\\ fail-ft_test_paths", 3);
	i = 0;
	while (back_slashed_paths[i])
	{
		accessible = ft_strjoin(back_slashed_paths[i], splitted_cmd[0]);
		if (!accessible)
			handle_error(splitted_cmd, back_slashed_paths, "malloc fail", 4);
	...
	handle_error(splitted_cmd, back_slashed_paths,
